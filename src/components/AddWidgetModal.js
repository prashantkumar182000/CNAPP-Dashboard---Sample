...
const AddWidgetForm = ({ onConfirm, onCancel }) => {
  ...
  const widgetCategories = [
    ...
    {
      label: 'Ticket',
      widgets: ['Ticketing System', 'Jira', 'ServiceNow'],
    }
  ];
  ...
  const handleConfirm = () => {
    ...
    const widgetObjects = widgetsArray.map((widget) => {
      ...
      if (widget === 'Cloud Accounts') {
        ...
        displayText = `Failed: ${inputValues[widget]?.Failed || 0}, Warning: ${inputValues[widget]?.Warning || 0}, Not Available: ${inputValues[widget]?.NotAvailable || 0}, Passed: ${inputValues[widget]?.Passed || 0}`;
        ...
        <Box key={widget} sx={{ marginTop: 2 }}>
          ...
          {widget === 'Cloud Account Risk Assessment' && (
            <Box sx={{ marginTop: 1 }}>
              ...
              <TextField
                type="number"
                value={inputValues[widget]?.Warning || 0}
                onChange={handleInputChange(widget, 'Warning')}
                InputProps={{ inputProps: { min: 0 } }}
              />
              <Typography variant="body2">Not Available:</Typography>
              <TextField
                type="number"
                value={inputValues[widget]?.NotAvailable || 0}
                onChange={handleInputChange(widget, 'NotAvailable')}
                InputProps={{ inputProps: { min: 0 } }}
              />
